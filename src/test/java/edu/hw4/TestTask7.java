...
public class TestTask7 {
    ...
    void Examples(List<Animal> animals, int k) {
        ...
        var result = Task7.kthOldestAnimal(animals, k);
        ...
        assertThat(result.age())
            .isEqualTo(animals.stream().map(Animal::age).sorted(Comparator.reverseOrder()).toList().get(k - 1));
    }
    ...
    void IncorrectInputTests(List<Animal> animals) {
        // then
        for (int k : List.of(-1, 0, 11)) {
            assertThatThrownBy(() -> Task7.kthOldestAnimal(animals, k))
